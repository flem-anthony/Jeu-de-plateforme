...
public class Plateforme
{
	...
	public void deplacer(char direction)
	{
		...
		/*-----------------------------------*/
		if (futureLig < 0                        ) futureLig = this.grille   .length - 1;
		if (futureCol < 0                        ) futureCol = this.grille[0].length - 1;
		if (futureLig > this.grille   .length - 1) futureLig = 0;
		if (futureCol > this.grille[0].length - 1) futureCol = 0;
		...
		if (getCaseEnvironnement(futureLig, futureCol) == 'r')
		{
			for (int cptLig = 0; cptLig < grille.length; cptLig++)
				for (int cptCol = 0; cptCol < grille[0].length; cptCol++)
					...
					if ( getCaseEnvironnement(cptLig, cptCol) == 'R' ||
					     getCaseEnvironnement(cptLig, cptCol) == 'r'    )
		...
		if (getCaseEnvironnement(futureLig, futureCol) == 'v')
		{
			for (int cptLig = 0; cptLig < grille.length; cptLig++)
				for (int cptCol = 0; cptCol < grille[0].length; cptCol++)
					...
					if ( getCaseEnvironnement(cptLig, cptCol) == 'V' ||
					     getCaseEnvironnement(cptLig, cptCol) == 'v'    )
							this.grille[cptLig][cptCol] = ' ';
		}
		...
		if (getCaseEnvironnement(futureLig, futureCol) == 'b')
		{
			for (int cptLig = 0; cptLig < grille.length; cptLig++)
				for (int cptCol = 0; cptCol < grille[0].length; cptCol++)
					...
					if ( getCaseEnvironnement(cptLig, cptCol) == 'B' ||
					     getCaseEnvironnement(cptLig, cptCol) == 'b'    )
							this.grille[cptLig][cptCol] = ' ';
		}
	}
}